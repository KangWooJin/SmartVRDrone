...
float ImageManager::thresh = 1;
float ImageManager::xscale = 0;
float ImageManager::yscale = 0;
float ImageManager::xshift = 0;
float ImageManager::yshift = 0;

void ImageManager::sampleImage(const IplImage* arr, float idx0, float idx1, CvScalar& res)
{
...
float ImageManager::getRadialX(float x,float y,float cx,float cy,float k)
{
      x = (x*xscale+xshift);
      y = (y*yscale+yshift);
...
float ImageManager::getRadialY(float x,float y,float cx,float cy,float k)
{
      x = (x*xscale+xshift);
      y = (y*yscale+yshift);
...
float ImageManager::calc_shift(float x1,float x2,float cx,float k)
{
      float x3 = x1+(x2-x1)*0.5;
      float res1 = x1+((x1-cx)*k*((x1-cx)*(x1-cx)));
      float res3 = x3+((x3-cx)*k*((x3-cx)*(x3-cx)));

      if(res1>-thresh and res1 < thresh) return x1;
      if(res3<0) return calc_shift(x3,x2,cx,k);
      else return calc_shift(x1,x3,cx,k);
}
Mat ImageManager::make3DImage(const Mat& img)
{
